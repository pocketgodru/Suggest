...
8.  [Дополнительные файлы данных](#дополнительные-файлы-данных)
	...
	- [Парсер данных Кинопоиска](#парсер-данных-кинопоиска)
...
Вы также можете самостоятельно собрать данные о фильмах с Кинопоиска с помощью встроенного парсера. Парсер находится в директории `kinopoisk_parser/` и предоставляет следующие возможности:
...
### Запуск парсера через Makefile (рекомендуется)
...
2. Запустите парсер:
```bash
python src/parser.py --mode all --output ../movie.json
```
...
# Запускаем полный цикл сбора данных
...
После успешного запуска парсера необходимо:
...
Подробная документация о парсере находится в файле `kinopoisk_parser/README.md`.
