...
При возникновении проблем с запуском или при работе обращаться в TG  [@Lord_Belemort](https://t.me/Lord_Belemort) , [@Save1iyy](https://t.me/Save1iyy) , [@stepprog](https://t.me/stepprog)
...
2. **Сгенерировать файлы с помощью парсера:**
   ...
   - Выполните следующие шаги:
     ...
     # Инициализировать базу данных MongoDB
     make init-db
...
5. (При запуске в Windows, не через WSL) Так как основными средами для разработки были Linux и MacOS, то при сборки надписи могут некорректно отображаться(неподдерживаемые кодировки), если сборка закончилась так:
```bash
[+] Running 5/5
 ✔ Container suggest-redis-1     Healthy                                                                                                                                                                         20.2s
 ✔ Container suggest-mongodb-1   Started                                                                                                                                                                         14.2s
 ✔ Container suggest-web-1       Started                                                                                                                                                                         33.8s
 ✔ Container suggest-database-1  Healthy                                                                                                                                                                         33.1s
 ✔ Container suggest-search-1    Started                                                                                                                                                                         15.1s
-e "\033[0;34mтЮд ╨Ю╨╢╨╕╨┤╨░╨╜╨╕╨╡ ╨╖╨░╨┐╤Г╤Б╨║╨░ MongoDB...\033[0m "
Непредвиденное появление: i.
make: *** [Makefile:45: run] Error 255
```
то она завершилась успешно, связано это с тем что Windows не поддерживает часть методов

6. Подождать несколько минут для синхронизации поисковиков
...
`При необходимости перезапустить контейнеры database и search-service`
...
Так же , так как модель используется через API, то при большом количестве одновременных запросов могут блокироваться вызовы, так же стоит подождать при запуске или после нескольких запросов 
